...
// 从环境变量读取配置
const ENABLE_CONTENT_SYNC = process.env.ENABLE_CONTENT_SYNC === "true"; // 默认关闭，显式开启才同步远程内容
...
try {
	...
	// 3. 提交主仓库
	if (process.env.CONTENT_SYNC_COMMIT === "true") {
		...
		console.log(
			"内容同步已完成，但未自动提交。设置 CONTENT_SYNC_COMMIT=true 可开启自动提交。",
		);
	}
} catch {
